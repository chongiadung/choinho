...
def get_status_code(url):
    try:
        ...
        print("Url %s has an invalid label" % url)
        return None
    except:
        return None
...
def setSpiderError(spider_name):
    ...
    error_number = spider['crawler_status'].get('error_number', 0) + 1
    if error_number >= 3:
        doc_status = 0
    else:
        doc_status = 1
    json_update = {
        'crawler_status.error_number': error_number,
        'crawler_status.status': 0 ,
        'doc.status' : doc_status,
        'crawler_status.last_stop_time' : now,
        'crawler_status.server_running' : None
    }
    return collection.update( {"doc.spider": spider_name} , {"$set":json_update},upsert=False, multi=False)        
  
def updateTimeStop(spider_name,items):
    now = time.time()
    json_update = {}
    json_update['crawler_status.error_number'] = 0
    json_update['crawler_status.last_stop_time'] = now
    json_update['crawler_status.status'] = 0
    json_update['crawler_status.server_running'] = None
    if items is not None:
        json_update['crawler_status.items'] = items
    return collection.update( {"doc.spider": spider_name} , {"$set":json_update},upsert=False, multi=False)

def updateTimeStart(spider_name):
    now = time.time()
    spider = collection.find_one({"doc.spider": spider_name})
    spider_status = spider['doc']['status']
    data_update = {}
    data_update['crawler_status.last_start_time'] = now
    data_update['crawler_status.server_running'] = SERVER.get(socket.gethostname(), socket.gethostname())
    data_update['crawler_status.status'] = 1
    if spider_status == 0 or spider_status == "0":
        data_update['doc.status'] = 1
    return collection.update( {"doc.spider": spider_name} , {"$set":data_update},upsert=False, multi=False)
...
def getFileNameFromSpiderName(spider_name):
    file_name = spider_name.replace('-', '').replace('www', '').replace('.', '')
...
TLD = [".com", ".com.vn", ".vn" , ".net", ".net.vn", ".org", ".pro.vn"]
...
def get_url_item(url_tracking):
    if "ho.lazada.vn" in url_tracking:
        return urlparse.parse_qs(urlparse.urlparse(url_tracking).query)['url'][0].split("?")[0]
    elif "click.accesstrade.vn" in url_tracking and "yes24.vn" in url_tracking:
        return urlparse.parse_qs(urlparse.urlparse(url_tracking).query)['url'][1]
    elif "click.accesstrade.vn" in url_tracking and "deca.vn" in url_tracking:
        return urlparse.parse_qs(urlparse.urlparse(url_tracking).query)['url'][0].split("?")[0]
    else:
        return url_tracking
